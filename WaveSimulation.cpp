...
// Wave simulation implementation
class WaveSimulation : public ISimulation {
private:
    int m_resolution = 200;
    float m_accumulator = 0.0f;
    float m_waveSpeedSquared = 0.01f; // c² for wave equation
    float m_simulationDt = 0.0f;
    float m_targetStepDt = 0.0f;

    void calculateTimestep() {
        float h = 1.0f / m_resolution;
        float waveSpeed = std::sqrt(m_waveSpeedSquared);
        float maxDt = (h / (waveSpeed * std::sqrt(2.0f))) * 0.9f;
        m_simulationDt = std::min(maxDt, m_targetStepDt);
    }

public:
    WaveSimulation(int resolution = 200) {
    ...
    void update(SimulationState& state, float& simTime, float simBudget) override {
        ...
        glUniform1f(state.uDiffusionU, m_waveSpeedSquared);
        glUniform1i(state.uComputeModeU, 1);
...
ISimulation* createWaveSimulation(int resolution) {
    ISimulation* sim = new WaveSimulation(resolution);
    sim->init(resolution);
    return sim;
}
