...
int runSimulation(int simulationMode)
{
  ...
  unsigned int rectProg = makeProgram({ compileShader(GL_VERTEX_SHADER, rectVertSrc.c_str()), compileShader(GL_FRAGMENT_SHADER, rectFragSrc.c_str()) });
...
int uTMRotationU = glGetUniformLocation(topomapProg, "uRotation");
int uTMResU = glGetUniformLocation(topomapProg, "uRes");
int uTMModeU = glGetUniformLocation(topomapProg, "uMode");
int uTMOpacityU = glGetUniformLocation(topomapProg, "uOpacity");
int uTMYOffsetU = glGetUniformLocation(topomapProg, "uYOffset");
